...
tput sgr0 #black
cdo mergetime chirps-v2.0.????.??.days_p05.nc chirps-v2.0.$((year-1))-$year.nd.days_p05.nc
cdo monmean chirps-v2.0.$((year-1))-$year.nd.days_p05.nc chirps-v2.0.$((year-1))-$year.nd.mons_p05.nc
rm -f chirps-v2.0.*.days_p05.nc  

tput setaf 1 #red
echo "Downloading ECMWF data for Feb"
tput sgr0 #black
python jan_forecast_data_download.py $year
...
tput sgr0 #black
python3.7 -W ignore jan_forecast_output.py $year
